module.exports.NAME = async function(req, res, next) {
  ...
  let revokeTime=0;
       ...
       const paramToEnroll = {
          'id_card': bodydata.identityValue,
        };
        ...
        if (respEnrollCheck.status == 200 && respEnrollCheck.data &&
          ...
          revokeTime = respEnrollCheck.data.resultData[0].revoke_time;
        }
  const query = {
  ...
  for (let i = 0; i< mongoResponse.length; i++) {
	...
	if(dataRequest.service_id=='verify' || (dataRequest.as_id_list && dataRequest.as_id_list[0] && dataRequest.as_id_list[0]=="F6977BA6-2525-4C86-9C1E-7ECB4B7DEE30")) {
